<div class="relative" wire:key="notification-bell">
    <div role="button"
         tabindex="0"
         wire:click="toggle"
         class="relative flex items-center justify-center w-10 h-10 rounded-full bg-slate-800 text-white hover:bg-slate-700 transition duration-200 cursor-pointer">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6 6 0 10-12 0v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/>
        </svg>
        @if ($this->unreadCount > 0)
            <span class="absolute -top-1 -right-1 min-w-[1.4rem] px-1.5 py-0.5 text-[10px] font-semibold leading-none text-white bg-red-600 rounded-full text-center">
                {{ $this->unreadCount }}
            </span>
        @endif
    </div>
    ...
    @if ($open)
        <div class="absolute right-0 mt-3 w-80 bg-white border border-slate-200 rounded-2xl shadow-xl z-50 overflow-hidden">
